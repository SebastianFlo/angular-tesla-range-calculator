...
// importing form accesories
import { ControlValueAccessor, NG_VALUE_ACCESSOR} from '@angular/forms';

// NUMBER_CONTROL_ACCESOR constant to allow us to use the "TeslaCounterComponent" as
// a custom provider to the component and enforce the ControlValueAccesor interface

const NUMBER_CONTROL_ACCESOR = {
  provide: NG_VALUE_ACCESSOR,
  // forwardRef allows usto grab the TypeScript class at a later (safer) point, as classes
  // aren't hoisted
...
@Component({
  ...
  `,
  // set the custom accesor as a provider
  providers: [NUMBER_CONTROL_ACCESOR],
...
export class TeslaCounterComponent implements ControlValueAccessor {
  ...
  // title to be passed to the counter__controls
  @Input() title: string = '';
  // unit to be used alonside the tile (mph/degree/anything)
  @Input() unit: string = '';

  value: number;
  focused: boolean;

  // internal functions to call when ControlValueAccesor gets called
  ...
  registerOnChange(fn: Function) {
    // assigns to our internal model change menthod
  ...
  // writes the value to the local component that binds to "value"
  ...
  // our onKeyUp event, will respong to User
